...
export default function PixelCard({ variant = 'default', gap, speed, colors, noFocus, className = '', children }) {
  ...
  const initPixels = () => {
    ...
    // Palette cam cho vòng tròn ở giữa (bạn có thể thay đổi các mã màu này theo ý thích)
    const orangePalette = ['#ffedd5', '#fdba74', '#f97316', '#ea580c'];
    ...
    const cx = width / 2;
    const cy = height / 2;
    // Bán kính hình tròn ở giữa (hiện đang đặt bằng 30% chiều rộng, bạn có thể tinh chỉnh lại)
    ...
    for (let i = 0; i < xValues.length; i++) {
      for (let j = 0; j < yValues.length; j++) {
        ...
        let drawX = x;
        let drawY = y;
        let isCorner = false; // Tắt hiệu ứng góc như yêu cầu
        ...
        const dx = drawX - cx;
        const dy = drawY - cy - bottomOffset;
        const distanceForDelay = Math.sqrt(dx * dx + dy * dy);
        const delay = reducedMotion ? 0 : distanceForDelay;

        // Khoảng cách thực tế từ pixel đến tâm
        const distFromCenter = Math.sqrt(Math.pow(drawX - cx, 2) + Math.pow(drawY - cy, 2));

        let color = colorsArray[Math.floor(Math.random() * colorsArray.length)];

        // Nếu pixel nằm trong bán kính hình tròn ở giữa, thay màu thành màu cam
        if (distFromCenter <= centerRadius) {
          color = orangePalette[Math.floor(Math.random() * orangePalette.length)];
        }
