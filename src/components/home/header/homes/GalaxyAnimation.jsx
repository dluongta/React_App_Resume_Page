...
const GalaxyAnimation = ({ text = "DINH LUONG TA", imageUrls = ["luen_logo.png", "hexagon-main.png", "luen-1.jpg"] }) => {
  ...
  useEffect(() => {
    ...
    const startUniverse = () => {
      ...
      // Set tốc độ rất chậm cho particle (như trong bản HTML chuẩn)
    ...
    // Fix độ nét Canvas (Anti-aliasing/High DPI)
    const resize = () => {
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        let p = particles[i];
        // Công thức giúp hạt xoay chậm (time * 0.1)
        ...
        if (item.type === 1) {
          ...
          if (scale > IMAGE_SHOW_SCALE) {
            ...
            if (img && img.complete && img.naturalWidth > 0) {
              ...
              // Thiết lập độ phân giải cao và filter hiển thị ảnh sắc nét
